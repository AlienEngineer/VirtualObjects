...
namespace VirtualObjects.Reflection
{
    internal static class MemberFilter
    {
        ...
        private static bool IsReservedName(this string name)
        {
            name = name.ToLowerInvariant();
            return name == ".ctor" || name == ".cctor";
        }

        private static string TrimExplicitlyImplementedName(this string name)
        {
        ...
        private static bool ContainsOverride<T>(this IList<T> candidates, MethodInfo method) where T : MemberInfo
        {
            ...
            for (int i = 0; i < candidates.Count; i++)
            {
                ...
                if (parameters.Select(p => p.ParameterType).SequenceEqual(candidate.GetParameters().Select(p => p.ParameterType)))
                {
                    return true;
                }
            }
